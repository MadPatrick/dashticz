...
var DashticzLayoutEditor = (function () {
  ...
  function _widgetIdFromReference(reference) {
    ...
    if (widgetReferences[ref]) return widgetReferences[ref];
    // Also recognise screen-specific variants generated by savewidgets.php for
    // non-primary screens (e.g. widget_clock_s2, widget_clock_standby).
    // We require the suffix to begin with '_' so that 'widget_clockX_s2' never
    // matches the 'widget_clock' entry.
    var keys = Object.keys(widgetReferences);
    for (var i = 0; i < keys.length; i++) {
      var base = keys[i];
      if (ref.slice(0, base.length) !== base) continue;
      var suffix = ref.slice(base.length);
      if (suffix === '_standby' || /^_s[0-9]+$/.test(suffix)) {
        return widgetReferences[base];
      }
    }
    return null;
  }
