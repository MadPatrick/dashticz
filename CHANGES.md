...
## 3.40.4 — Layout editor: widget config cog opens Widget Config for all screens

- Fixed a bug where clicking the config cog on a widget tile (clock, weather,
  calendar, etc.) on any non-primary screen (screen 2, standby, …) silently
  opened Device Config instead of Widget Config, or did nothing at all.
  Root cause: `_widgetIdFromReference` in `layouteditor.js` only recognised
  exact block keys like `widget_clock`; screen-specific variants generated by
  `savewidgets.php` (`widget_clock_s2`, `widget_clock_standby`, etc.) were not
  in the lookup table, so `_resolveBlock` returned `null` for those references.
  On grid screens the fallback created an item with `kind: 'grid'` and no
  `widgetId`, which caused the cog to call Device Config instead.  On column
  screens the item was skipped entirely and no cog was rendered.
  Fix: `_widgetIdFromReference` now strips the recognised screen-specific
